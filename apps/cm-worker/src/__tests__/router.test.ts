...
function makeFinding(overrides: Partial<CmFinding>): CmFinding {
  return {
    ...
    description: null, taintFlow: null,
    createdAt: "", updatedAt: "",
    ...overrides,
  };
}

const criticalSca = makeFinding({ id: "1", source: "sca", severity: "CRITICAL", fingerprint: "fp1" });
const highSast = makeFinding({ id: "2", source: "sast", severity: "HIGH", rule: "SQL Injection", file: "src/users.ts", line: 42, package: null, currentVersion: null, fixedVersion: null, fingerprint: "fp2" });
const mediumSca = makeFinding({ id: "3", source: "sca", severity: "MEDIUM", rule: "CVE-3", package: "minimist", currentVersion: "1.2.5", fixedVersion: "1.2.8", fingerprint: "fp3" });
const lowSast = makeFinding({ id: "4", source: "sast", severity: "LOW", rule: "Debug Endpoint", file: "src/debug.ts", line: 10, package: null, currentVersion: null, fixedVersion: null, fingerprint: "fp4" });

describe("filterAndRouteFindings", () => {
  it("splits mixed findings into SCA and SAST", () => {
    const result = filterAndRouteFindings([criticalSca, highSast], ["CRITICAL", "HIGH"]);
  ...
  it("skips below-threshold findings", () => {
    const result = filterAndRouteFindings([criticalSca, mediumSca, lowSast], ["CRITICAL", "HIGH"]);
  ...
  it("marks skipped findings with skipped fixStatus", () => {
    const result = filterAndRouteFindings([criticalSca, mediumSca], ["CRITICAL"]);
  ...
  it("includes all findings when threshold is very low", () => {
    const result = filterAndRouteFindings([criticalSca, highSast, mediumSca, lowSast], ["LOW"]);
