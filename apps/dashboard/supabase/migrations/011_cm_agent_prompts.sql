...
-- Mirrored at supabase/migrations/20260614_cm_agent_prompts.sql
...
BEGIN
  SELECT id INTO cm_cat_id FROM agent_categories WHERE slug = 'cm' LIMIT 1;
...
$verifier$;
  ...
  -- Insert rows first if they don't exist
  ...
  -- Now UPDATE all 4 agents with new prompts (works for both newly inserted and pre-existing rows)
